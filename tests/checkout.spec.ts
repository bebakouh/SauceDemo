...
test.describe('Phase 4 - Checkout Tests', () => {
  ...
  test('TC-CHECKOUT-002 - Erfolgreicher Checkout', async ({ page }) => {
    ...
    await checkout.fillCheckoutInformation('Max', 'Mustermann', '41061');

    await checkout.finishCheckout();

    await expect(checkout.completeTitle).toHaveText('Thank you for your order!');
  });

  test('TC-CHECKOUT-003 - Checkout Übersicht korrekt', async ({ page }) => {
    const checkout = new CheckoutPage(page);

    await checkout.fillCheckoutInformation('Test', 'Testing', '41065');

    await expect(page).toHaveURL(/checkout-step-two.html/);

    await expect(checkout.overviewTitle).toHaveText('Checkout: Overview');
